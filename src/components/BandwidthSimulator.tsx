...
export const BandwidthSimulator: React.FC<BandwidthSimulatorProps> = ({ 
  downlinkSpeed, 
  effectiveType,
  isMobileOpen = false,
  onMobileClose
  ...
  // Render the core form controls and simulator readouts
    ...
    <>
      ...
      <div className="border border-purple-100 rounded-xl p-3 bg-purple-50/50 space-y-3">
        ...
        <div className="text-[9px] text-slate-400 leading-normal border-t border-purple-100/50 pt-2 italic">
          ℹ️ Based on demo data — adapts reactively to save bandwidth.
        </div>
      </div>

      <div className="text-[10px] text-slate-500 leading-normal bg-slate-50 p-2 rounded-lg border border-slate-100">
        💡 <strong className="text-slate-700">Simulate Throttling:</strong> Use Chrome DevTools <strong className="text-blue-600">Network</strong> tab and select <strong className="text-amber-600">Fast 3G</strong>.
      </div>
    </>
  );
  ...
  return (
    <>
      {/* Desktop view: Absolute positioned on the top left */}
      <div className="hidden md:block absolute top-4 left-4 z-40 w-80 glass-panel rounded-2xl shadow-xl overflow-hidden no-print border border-purple-200 bg-white/90 text-slate-800">
        <button 
      ...
      {/* Mobile view: Modal overlay when triggered */}
      {isMobileOpen && (
        <div className="md:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0" onClick={onMobileClose} />
          <div className="relative z-10 w-full max-w-sm bg-white rounded-3xl border border-purple-200 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="px-4 py-3 flex items-center justify-between border-b border-purple-100 bg-purple-50/50">
              <div className="flex items-center gap-2">
                <Activity className="w-5 h-5 text-blue-600 animate-pulse" />
                <span className="font-bold text-slate-800 text-sm">Connection Status</span>
              </div>
              <button 
                onClick={onMobileClose}
                className="px-2.5 py-1 text-xs font-bold text-slate-500 hover:text-slate-850 hover:bg-slate-100 rounded-lg transition-all"
              >
                Close
              </button>
            </div>
            ...
            <div className="p-4 space-y-4 overflow-y-auto">
              <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                <span className="text-xs text-slate-500 font-bold">Current Applied:</span>
                <span className={`px-2 py-0.5 text-xs font-bold rounded-full border ${getTierColor(bandwidthTier)}`}>
